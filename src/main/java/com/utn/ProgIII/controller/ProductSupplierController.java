...
public class ProductSupplierController {
    ...
    @Operation(
            summary = "Busca todos los proveedores de un producto según su nombre",
            description = "Devuelve la lista de proveedores asociados a un producto dado su nombre"
    )
    @ApiResponse(
            responseCode = "200",
    ...
    @ApiResponse(
            ...
            content = @Content(
                    mediaType = "text/plain;charset=UTF-8",
                    schema = @Schema(example = "El producto no existe")
            )
    )
    @GetMapping("/filter/product/{productName}")
    public ResponseEntity <List<SupplierProductListDTO>> listAllSuppliersByProduct(@PathVariable String productName) {

        List<SupplierProductListDTO> response = productSupplierService.listSuppliersByProduct(productName);
        return ResponseEntity.ok(response);
    }
